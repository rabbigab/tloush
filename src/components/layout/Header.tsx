    ...
    <header className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-neutral-100 shadow-sm">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
        ...
        <nav className="flex items-center gap-1 sm:gap-2">
          <Link href="/privacy" className="flex items-center gap-1.5 text-xs sm:text-sm text-neutral-500 hover:text-brand-600 transition-colors px-2 py-1.5 rounded-lg hover:bg-brand-50">
            <ShieldCheck size={14} />
            <span className="hidden sm:inline">Confidentialité</span>
          </Link>
          <Link href="/analyze" className="btn-primary text-sm py-2 px-4">Analyser ma fiche</Link>
        </nav>
      </div>
    </header>
  );
