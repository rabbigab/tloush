...
export const metadata: Metadata = {
  ...
  description: "Guides complets en français sur vos droits : congés, heures supplémentaires, licenciement, cotisations. Tout ce qu'un salarié francophone doit savoir en Israël.",
};
    ...
    <main className="min-h-screen flex flex-col bg-neutral-50">
      ...
      <div className="flex-1 max-w-4xl mx-auto w-full px-4 sm:px-6 py-10">
        ...
        <div className="text-center mb-10">
          ...
          <h1 className="text-3xl sm:text-4xl font-bold text-neutral-900 mb-3">Vos droits en tant que salarié</h1>
          <p className="text-neutral-500 max-w-xl mx-auto text-sm">
            Tout ce que vous devez savoir sur le droit du travail israélien, expliqué en français.
            Mis à jour régulièrement avec les dernières lois en vigueur.
          </p>
        </div>
        ...
        <div className="flex flex-wrap gap-2 mb-8 justify-center">
          {CATEGORIES.map((cat) => {
            ...
            return (
              <Link
                key={cat.value}
                href={cat.value === "all" ? "/droits" : `/droits?categorie=${cat.value}`}
                className={`flex items-center gap-1.5 text-sm px-4 py-2 rounded-full border transition-colors ${isActive ? "bg-brand-600 text-white border-brand-600" : "bg-white text-neutral-600 border-neutral-200 hover:border-brand-300"}`}
              >
            ...
            >
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-neutral-500 line-clamp-2 mb-3">{guide.subtitle}</p>
                  <div className="flex items-center justify-between">
                    ...
                    <ChevronRight size={14} className="text-brand-400 group-hover:translate-x-0.5 transition-transform" />
                  </div>
                </div>
              </div>
            </Link>
        ...
        <div className="mt-12 bg-brand-50 border border-brand-100 rounded-2xl p-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div>
            <p className="font-semibold text-neutral-800 mb-1">Passez à l'action</p>
            <p className="text-sm text-neutral-500">Analysez votre fiche de paie pour vérifier que tout est conforme.</p>
          </div>
