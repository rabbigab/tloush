...
function renderMarkdown(md: string): string {
  return md
    .replace(
      /^## (.+)$/gm,
      '<h2 class="text-lg font-bold text-neutral-800 mt-8 mb-3">$1</h2>'
    )
    .replace(/^\| (.+) \|$/gm, (line) => {
      if (line.includes("---")) return "";
      const cells = line.split("|").filter((c) => c.trim());
      const isHeader =
        md.indexOf(line) < md.indexOf("|---|") || false;
      const tag = "td";
      return (
        "<tr>" +
        cells
          .map(
            (c) =>
              `<${tag} class="px-3 py-2 text-sm border border-neutral-200">${c.trim()}</${tag}>`
          )
          .join("") +
        "</tr>"
      );
    ...
    .replace(
      /(<li.*<\/li>\n?)+/g,
      (items) => `<ul class="my-3 space-y-1">${items}</ul>`
    )
    .replace(
      /⚠️ (.+)/g,
      '<div class="flex gap-2 items-start bg-warning/10 border border-warning/20 rounded-xl p-3 my-4"><span>⚠️</span><p class="text-sm text-neutral-700">$1</p></div>'
    )
    .replace(
      /\n\n/g,
      '</p><p class="text-neutral-700 text-sm leading-relaxed my-3">'
    )
    .replace(
      /^(?!<)(.+)$/gm,
      '<p class="text-neutral-700 text-sm leading-relaxed my-3">$1</p>'
    );
}
            ...
            <div>
              ...
              <div className="flex items-center gap-4 mt-3 text-xs text-neutral-400">
                <span className="flex items-center gap-1">
                  <Clock size={11} /> {guide.readingTime} de lecture
                </span>
                <span className="flex items-center gap-1">
              ...
              {related.map((r) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-neutral-800 group-hover:text-brand-700 transition-colors truncate">
                      {r.title}
                    </p>
                    <p className="text-xs text-neutral-400">
                      {r.readingTime} de lecture
                    </p>
                  </div>
                  <ChevronRight
                    size={14}
